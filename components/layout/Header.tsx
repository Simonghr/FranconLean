...
export function Header({ onMenu }: { onMenu?: () => void }) {
  ...
  return (
    <header className="h-14 bg-slate-900 border-b border-slate-800 flex items-center justify-between px-4 md:px-6 flex-shrink-0 gap-2">
      <div className="flex items-center gap-2 md:gap-3 min-w-0">
        <button
          onClick={onMenu}
          className="md:hidden text-slate-400 hover:text-white p-1.5 -ml-1 rounded-lg hover:bg-slate-800 transition-colors flex-shrink-0"
          title="Menu"
        >
        ...
        <span className="text-sm text-slate-400 hidden sm:inline">{dateFormatted}</span>
      </div>
